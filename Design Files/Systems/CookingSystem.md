...
**<u>Other Information:</u>** Cooking does not take any amount of in-game time. Time pauses when the Cooking Menu is open and does not restart until after the Meal or Drink has completed. Recipes are only shown as available if they are both discovered and if the player has all necessary ingredient items. The player cannot cancel once confirming all ingredients added to the Stove or Blender. When consuming a Meal or Drink, the player learns that recipe that can be made.
